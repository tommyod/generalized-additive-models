...
# Fit a model
spline = Spline(0)
gam = GAM(spline, fit_intercept=True, verbose=3)
gam.fit(X, y)
...
upper = y_smooth + stdev_array
lower = y_smooth - stdev_array
plt.fill_between(x_smooth, lower, upper, zorder=5, alpha=0.33, label="±1 std")
